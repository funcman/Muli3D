...
/// @file m3dcore_texture.h
///

#ifndef __M3DCORE_TEXTURE_H__
#define __M3DCORE_TEXTURE_H__
...
class CMuli3DTexture : public IMuli3DBaseTexture
{
protected:
	~CMuli3DTexture(); ///< Accessible by IBase. The destructor is called when the reference count reaches zero.
	...
	result Create( uint32 i_iWidth, uint32 i_iHeight, uint32 i_iMipLevels,
		m3dformat i_fmtFormat );

	m3dtexsampleinput eGetTexSampleInput(); ///< Sampling this texture requires 2 floating point coordinates.

	friend class CMuli3DCubeTexture;
	/// Accessible by CMuli3DDevice and CMuli3DCubeTexture. (A cube-texture consists of six standard 2d textures and needs access to their sampling function.)
	...
	/// @param[in] i_fW w-component of the lookup-vector (unused).
...
public:
	...
	/// @return s_ok if the function succeeds.
	/// @return e_invalidparameters if the clear-rectangle exceeds the texture's dimensions.
	...
	/// @param[in] i_iMipLevel mip-level, 0 being the largest mip-level.
	class CMuli3DSurface *pGetMipLevel( uint32 i_iMipLevel );
...
private:
	...
	float32					m_fSquaredWidth, m_fSquaredHeight; ///< Squared dimensions of the base mip-level, used for mip-calculations.
	class CMuli3DSurface	**m_ppMipLevels;		///< Pointer to the mip-level data.
};

#endif // __M3DCORE_TEXTURE_H__
